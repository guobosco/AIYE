...
aiye 是一个跨平台的服务与社交应用，旨在通过服务发现、心愿单、实时消息等功能连接用户。本项目采用 Monorepo（单体仓库）结构进行管理，同时包含了统一的后端服务以及 Android、iOS 和微信小程序三个客户端端点。
...
├── android_aiye/     # 原生 Android 客户端 (Kotlin + Jetpack Compose)
├── ios_aiye/         # 原生 iOS 客户端 (SwiftUI)
└── wechat_aiye/      # 微信小程序端 (Taro + React)
```
...
2. **安装依赖**：
   ...
   ```
3. **配置接口地址**：
   - 打开 `wechat_aiye/config/dev.ts` 文件。
   - 将 `TARO_APP_API_BASE_URL` 修改为 `https://123.57.67.153`。
4. **启动开发服务**：
   ```bash
   npm run dev:weapp
   ```
5. 打开 **微信开发者工具**，导入 `wechat_aiye` 目录，即可开始调试。
...
- **Android**: `android_aiye/gradle.properties` 中的 `DEV_API_BASE_URL`
