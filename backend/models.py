...
class User(Base):
    ...
    # 登录来源（password | wechat）
    login_provider = Column(String, default="password")
    # 微信跨应用统一标识
    wechat_unionid = Column(String, index=True, nullable=True)
    # 微信应用内标识
    wechat_openid = Column(String, index=True, nullable=True)
    # 最近一次微信登录平台（android | ios）
    wechat_platform = Column(String, default="", nullable=False)
    # 手机号绑定时间
    phone_bound_at = Column(BigInteger, default=0)
    # 最近登录时间
    last_login_at = Column(BigInteger, default=0)
    ...
    # 收藏的体验 ID 列表
    favorite_experience_ids = Column(JSON, default=list)
    # 个性签名
...
class Experience(Base):
    ...
    cover_image_url = Column(String, default="")
    image_urls = Column(JSON, default=list)
    detail_steps = Column(JSON, default=list)
    location = Column(String, default="")
    price_text = Column(String, default="")
    price_basis_text = Column(String, default="")
