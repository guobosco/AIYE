...
const summarySceneSignalGroups: Array<{ label: string; keywords: string[] }> = [
  { label: '亲子出游', keywords: ['亲子', '家庭', '宝宝', '小朋友'] },
  { label: '情侣约会', keywords: ['情侣', '约会', '纪念日'] },
  { label: '聚会团建', keywords: ['团建', '聚会', '派对', '生日', '求婚', '婚礼'] },
  { label: '新手友好', keywords: ['新手', '零基础', '初学', '第一次'] },
  { label: '行程可定制', keywords: ['定制', '定做', '定制化', '灵活调整', '可调整'] },
];

const featureSignalGroups: Array<{ label: string; keywords: string[] }> = [
  { label: '新手友好', keywords: ['新手', '零基础', '初学', '第一次'] },
  { label: '亲子友好', keywords: ['亲子', '家庭', '宝宝', '小朋友'] },
  { label: '情侣适合', keywords: ['情侣', '约会', '纪念日'] },
  { label: '团建适合', keywords: ['团建', '聚会', '派对', '生日', '求婚', '婚礼'] },
  { label: '可上门', keywords: ['上门', '到家', '到府'] },
  { label: '到店服务', keywords: ['到店', '门店', '工作室'] },
  { label: '线上沟通', keywords: ['线上', '视频', '远程'] },
  { label: '可定制', keywords: ['定制', '定做', '定制化', '灵活调整', '可调整'] },
  { label: '夜间友好', keywords: ['夜景', '夜拍', '夜间', '晚上'] },
  { label: '英文沟通', keywords: ['英语', '英文'] },
  { label: '一对一', keywords: ['一对一', '私教', '单独指导'] },
  { label: '会引导', keywords: ['引导动作', '指导动作', '不会摆拍'] },
  { label: '路线熟悉', keywords: ['熟悉路线', '带路', '安排行程', '路线规划'] },
  { label: '当日返图', keywords: ['当日返图', '当天返图', '加急返图', '快修'] },
  { label: '可修图', keywords: ['修图', '精修'] },
  { label: '情绪价值', keywords: ['情绪价值', '会聊天', '陪聊', '健谈'] },
  { label: '控场互动', keywords: ['控场', '暖场', '互动', '带动气氛'] },
  { label: '忌口可调', keywords: ['忌口', '口味调整'] },
];

const featureTagPriority = [
  '可上门',
  '会引导',
  '路线熟悉',
  '一对一',
...
const synonymousTagGroups = [
  ['新手友好', '零基础友好'],
  ['可定制', '行程可定制', '曲风可定制', '可定制菜单'],
  ['亲子出游', '亲子友好'],
  ['情侣约会', '情侣适合'],
  ['聚会团建', '团建适合'],
  ['会引导', '动作示范', '表情引导'],
...
const locationSignalGroups: Array<{ label: string; keywords: string[] }> = [
  { label: '亚龙湾', keywords: ['亚龙湾'] },
  { label: '椰梦长廊', keywords: ['椰梦长廊'] },
  { label: '蜈支洲', keywords: ['蜈支洲'] },
  { label: '海棠湾', keywords: ['海棠湾'] },
  { label: '大东海', keywords: ['大东海'] },
  { label: '三亚湾', keywords: ['三亚湾'] },
  { label: '海边', keywords: ['海边', '海岛'] },
  { label: '三亚', keywords: ['三亚'] },
];

const searchableBundle = (...values: Array<string | null | undefined>) =>
  values
...
const extractSignalLabel = (
  groups: Array<{ label: string; keywords: string[] }>,
  text: string
) => groups.find(({ keywords }) => keywords.some((keyword) => text.includes(keyword)))?.label ?? '';

const isSynonymousTag = (value: string, existing: string[]) =>
  synonymousTagGroups.some((group) => group.includes(value) && existing.some((item) => group.includes(item)));

const sortFeatureTags = (values: string[]) =>
  [...values].sort((left, right) => {
    const leftIndex = featureTagPriority.indexOf(left);
    const rightIndex = featureTagPriority.indexOf(right);
...
const buildLocationTag = (location?: string | null, searchableText?: string) => {
  const normalizedLocation = normalizeText(location);
  const searchText = searchableText ?? '';
  const matchedSignal = locationSignalGroups.find(({ keywords }) =>
    keywords.some((keyword) => normalizedLocation.includes(keyword) || searchText.includes(keyword))
  )?.label;
  if (matchedSignal) {
    return matchedSignal;
  }
  if (normalizedLocation && normalizedLocation.length <= 8) {
    return normalizedLocation;
  }
  if (searchText.includes('上门') || searchText.includes('到家') || searchText.includes('到府')) return '可上门';
  if (searchText.includes('到店') || searchText.includes('门店') || searchText.includes('工作室')) return '到店服务';
  if (searchText.includes('线上') || searchText.includes('远程') || searchText.includes('视频')) return '线上沟通';
  return '';
};

const buildBookingTag = (leadHours?: number, futureOpenDays?: number) => {
  const hours = Number(leadHours ?? 0);
  const days = Number(futureOpenDays ?? 0);
  if (Number.isFinite(hours)) {
    if (hours <= 0) return '当天可约';
    if (hours <= 6) return '提前6小时';
    if (hours <= 24) return '提前1天';
    if (hours <= 48) return '提前2天';
  }
  if (Number.isFinite(days) && days >= 45) return '长期可约';
  return '';
};

const buildPrepaymentTag = (prepaymentPercent?: number) => {
  const percent = Number(prepaymentPercent ?? 0);
  if (!Number.isFinite(percent)) return '';
  if (percent <= 0) return '无需预付';
  return `${percent}%预付`;
};

const buildSummaryTags = (data: any) => {
  const searchText = searchableBundle(
    data.title,
    data.description,
    data.location,
    data.service_mode,
    ...(Array.isArray(data.service_declarations_extra) ? data.service_declarations_extra : [])
  );
  return uniqueNonEmpty([
    buildLocationTag(data.location, searchText),
    extractSignalLabel(summarySceneSignalGroups, searchText),
    buildBookingTag(data.booking_lead_hours, data.booking_future_open_days),
    buildPrepaymentTag(data.prepayment_percent),
  ]).slice(0, 4);
};
...
const buildServiceTypeKeywords = (data: any) => {
  const selectedFeatureTags = Array.isArray(data.service_feature_tags)
    ? data.service_feature_tags.map((item: unknown) => normalizeText(typeof item === 'string' ? item : ''))
    : [];
  const searchableText = searchableBundle(
    data.title,
    data.description,
    data.location,
    data.service_mode,
    data.price_basis_text,
    ...(Array.isArray(data.service_declarations_extra) ? data.service_declarations_extra : [])
  );
  const matchedSignals = featureSignalGroups
    .filter(({ keywords }) => keywords.some((keyword) => searchableText.includes(keyword)))
    .map(({ label }) => label);
  const summaryTags = buildSummaryTags(data);
  const deduped: string[] = [];
  sortFeatureTags(uniqueNonEmpty([...selectedFeatureTags, ...matchedSignals])).forEach((tag) => {
    if (summaryTags.includes(tag) || isSynonymousTag(tag, summaryTags) || isSynonymousTag(tag, deduped)) return;
    deduped.push(tag);
  });
